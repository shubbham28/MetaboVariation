...
#' @description
#' Plots a heatmap that shows the number of flagged individuals in which the pair of metabolites contributes to flagging individuals within a time point.
#'
...
#' @param threshold A positive integer indicating the minimum number of metabolites an individual must be flagged in a time point to be included in the heatmap.
#'
#' @return Returns a heatmap that illustrates the number of individuals flagged in the pair-wise metabolites within a time point.
#' @export
#'
#' @examples
#' \dontrun{
...
#' metabolitepair.heatmap(model,interval=0.975,cutoff = 3)
#'}

metabolitepair.heatmap <-function(model,interval,threshold){
  ...
  flagged_dep = model %>%  dplyr::filter(.data[[paste0("flag",interval)]] == 1) %>%   dplyr::group_by(individual,timepoint) %>%  dplyr::summarise(flagged_metabolites = dplyr::n_distinct(metabolite),
                                                                                                                                                  metabolites_list = toString(sort(unique(metabolite)))) %>% dplyr::filter(flagged_metabolites > threshold)
  ...
  ordered_co_occurrence_matrix <- co_occurrence_matrix[clustering$order, clustering$order]
