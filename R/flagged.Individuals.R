...
#' @description
#' A function to list the flagged individuals resulting from fitting a Bayesian generalised linear model (BGLM) to repeated measures data.
#'
...
#' @param threshold A positive integer. Individuals that have been flagged in less than _threshold_ times across different metabolites will not be included. By default, it is set to 2.
#'
#' @return The function will provide two types of lists based on the type of model object.
#' \itemize{
#'   \item Single metabolite model - if the model contains result for a single metabolite, the function will return a list of flagged individuals along with their metabolite levels across different time points.
#'   \item Multiple metabolites model - if the model contains results for multiple metabolites, the function will return a list of individuals flagged in those metabolites  along with the number of times that individual has been flagged in each metabolite.
#' }
...
#' flagged.Individuals(model,data = metabol.data,
#' individual_id = individual_id)
#' }
