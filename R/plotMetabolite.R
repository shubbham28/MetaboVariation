...
#' @description
#' A function that displays the distribution of a metabolite levels for all individuals across all time points as a violin plot.
#'
...
#'
#' @return It returns a violin plot for the metabolites passed.
#' @export
#'
#' @examples
#' data(metabol.data)
...
#' plotMetabolite(data = metabol.data,metabolite = metabolites[1:3])
#'
plotMetabolite <- function(data, metabolite){
  ...
  if (length(metabolite)>1){
    for (item in metabolite) {
      ...
      print(new_data %>% plotly::plot_ly(x=~occurrence,y=~measurement,split=~occurrence,type="violin",points = FALSE) %>% plotly::layout(title = paste("Density of different timepoints for", item),
    ...
    new_data = reshape2::melt(data,measure.vars = plot_list,variable.name = "occurrence",value.name = "measurement")
    new_data %>% plotly::plot_ly(x=~occurrence,y=~measurement,split=~occurrence,type="violin",points = FALSE) %>% plotly::layout(title = paste("Density of different timepoints for", metabolite),
