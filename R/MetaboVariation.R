...
#' @param covariates A list of strings containing the names of columns in the data that contain the covariates.
...
#' @param iter The number of total iterations per chain (including warmup; defaults to 2000).
...
#' @param cutoff A list of cutoffs for the width of the highest posterior distribution interval of the posterior predictive distribution. The value should be less than 1. Default cutoffs are set to 0.95, 0.97, and 0.99.
...
#' @param n_chains Number of chains to use when fitting the model.
#' @param prior A list of prior specifications with 3 elements: R (random effects of individuals), G (random errors of the model), B (fixed effects). B is a list containing the expected value (mu) and a (co)variance matrix (V) representing the strength of belief. The priors for the variance structures (R and G) are lists with the expected (co)variances (V) and a degree of belief parameter (nu) for the inverse-Wishart, where nu should be greater than the number of metabolites.
...
#' @references Andrew Gelman, Donald B. Rubin "Inference from Iterative Simulation Using Multiple Sequences," Statistical Science, Statist. Sci. 7(4), 457-472,
...
MetaboVariation <- function ( data,individual_ids,metabolites,covariates=NULL,type="dependent",iter = 5000,
                                warmup = 2000,thin = 2,cutoff=c(0.95,0.975,0.99),cores = NULL,n_chains=4,prior=NULL){
    # if(!is.null(seed)){
    #   if(!is.numeric(seed)){
    ...
    dependent_result<-foreach::foreach(i=0:obs,.combine=rbind,.packages = c('MCMCglmm'))%dopar%{
      ...
      result = prediction(sub_model,newdata = final_data[drop,],levels = cutoff,prior=prior)$summary
