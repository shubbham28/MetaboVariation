...
#' \itemize{
#'   \item Circos plot - A circular plot that will show the posterior predictive credible interval for all the individuals across timepoints. If the observed value
...
plot.MetaboVariation <- function(x,type = "circos",timepoints = NULL,...){
  ...
  if(inherits(model,"meta.single_model")){
    ...
    size[is.na(size)] = 1
    size = apply(size[,-1],1,function(x){sum(x)%/%length(unique(c(1,2,3,4)))})
