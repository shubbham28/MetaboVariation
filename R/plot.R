...
#' A circos plot or a barplot can be produced depicting results from different perspectives.
#'
...
#' \itemize{
#'   \item Circos plot - A circular plot that will show the credible interval for all the individuals across timepoints. If the observed value
#'   is outside the credible interval, that bar will be flagged as black and the individual's id will be shown in __bold__.
#'   \item Metabolites count plot - A bar plot that shows the number of individuals that are flagged in each timepoint for all metabolites passed.
#' }
  ...
  if(inherits(model,"meta.single_model")){
    ...
    if(is.null(timepoints)){
      ...
      for (i in 2:ncol(values)) {
        ...
        lgd = ComplexHeatmap::Legend(at = paste0("Timepoint",1:(ncol(values)-1)), type = "lines", legend_gp = grid::gpar(col = colors[2:ncol(values)], lwd = 3), title_position = "topleft", title = "Timepoints", ncol = 4)
    ...
    if(type == "metabolites_count"){
      ...
      plotly::plot_ly(sum_plot,x = ~Timepoint, y = ~sum_m, type = "bar",marker = list(color=colors[1],width = 0.2)) %>%
        plotly::layout(yaxis = list(title = 'No. of individuals flagged'))
  ...
  # Multiple metabolite model
    ...
    #  sum_plot = reshape2::melt(sum_plot,id.vars = "Metabolites", variable.name = "Timepoints",value.name = "count")
     ...
     figure = plotly::plot_ly(x = sum_plot[["Metabolites"]],y = sum_plot[[paste0("Timepoint.",timepoints[1])]], type = "bar",marker = list(color=colors[timepoints[1]]),width = 0.2,name = paste0("Timepoint ",timepoints[1])) %>%
      plotly::layout(yaxis = list(title = 'No. of individuals flagged',range = c(min(sum_plot[,-1]) - 1,max(sum_plot[,-1]) + 1)), barmode = 'group')
